# Experimentation on "bollingerWithRSI.py" to increase the score/returns.

"""
    Title: Bollinger Band Strategy with RSI (NSE)
...
def initialize(context):
    ...
    """
    # universe selection
    context.securities = [symbol('ADANIENT'), symbol("TATAELXSI"), symbol("BRITANNIA")]
    ...
    # volatility
    context.volatility = dict((security, 1) for security in context.securities)
...
def generate_target_position(context, data):
    ...
    for security in context.securities:
        print("\nfor ", security)
        print("volatility: ", context.volatility[security])
        scale = abs(context.volatility[security])
...
def signal_function(px, params, context, security):
    ...
    # print("Bollinger" + "upper: " + str(upper) + "mid: " + str(mid) + "lower: " + str(lower))
    # print("Bollinger=> upper: %d , mid: %d, lower: %d" % (upper, mid, lower))
    ...
    # print("dist_to_upper: ", dist_to_upper)

    # measure of volatility
    try:
        context.volatility[security] = (upper - last_px)/(upper - lower)    # b/w (0, 1)
    except:
        context.volatility[security] = 1
        print("couldn't cal volat")
    # print("\nvolatility: ", context.volatility[security])
    
    # why not just decide betting size using this?

    if dist_to_upper > 95:
        # return (dist_to_upper - 100) / 10 - 0.5
        # return (90 - dist_to_upper) / 10
        return -1
    elif dist_to_upper < 5:
        # return 0.5 + (dist_to_upper / 10)
        # return (10 - dist_to_upper) / 10
        return 1
